...
Periodicity:
    Since the period of a state is the largest $d$ that satisfy following properties: 
      ...
      - If $p_{ii}^{(n)} = 0$, for all $n>0\rightarrowd(i)=\infty$
      and...
...
Transient or recurrent? $\rightarrow$ It is recurrent!
  ...
  $f_{ii}=P(X_{n} = i, \textrm{for some } n≥1|X_{0}=i)$ for any state $i$ is Recurrent if $f_{ii}=1$ and transient if Recurrent if $f_{ii}<1$.
